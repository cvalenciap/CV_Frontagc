...
import { ParametrosCargaBandeja,RepoInfActiEjec, Paginacion, Response} from 'src/app/models';
...
@Component({
  selector: 'app-informe-actividad-ejecutada',
  templateUrl: './informe-actividad-ejecutada.component.html',
  styleUrls: ['./informe-actividad-ejecutada.component.scss']
})
export class InformeActividadEjecutadaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.myForm = this.fb.group({
      data: { year: this.anio, month: this.mes }
    });
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarPDF(this.items).subscribe(
      (data) => {
        console.log(data);
  ...
  OnFiltrar(){
    this.isLoading = true;
    const this_ = this;
   // console.log(this.ObtenerReportesRequest());
  ...
  OnLimpiar(){
    ...
    this.mes = (new Date()).getMonth() + 1;
    this.anio = (new Date()).getFullYear();
    this.myForm.controls['data'].setValue({ year: this.anio, month: this.mes });
  }
  ...
  obtenerSubactividad(){
    const this_ = this;
    this.reportesMonitoreoService.obtenerSubactividades(this.ObtenerReportesRequest()).subscribe(
      (response: Response) => {
        if(response.resultado.length!=''){
          this_.subaticList = [];
          this_.listSubacti = [];
          for(let i=0; i<response.resultado.length;i++){
           ...
           this_.listSubacti.push(JSON.parse(JSON.parse(JSON.stringify('{"title": "TODOS", "key": ""}'))));
