...
@Component({
  selector: 'app-efectividad-lector-toma-estado',
  templateUrl: './efectividad-lector-toma-estado.component.html',
  styleUrls: ['./efectividad-lector-toma-estado.component.scss']
})
export class EfectividadLectorTomaEstadoComponent implements OnInit {
  ...
  OnFiltrar(){
    ...
    request.v_codigo_lector = this.lector==''?null:this.lector;
    this.reportesMonitoreoService.obtenerRepoEfecLectorTomaEst(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarEfectividadTomaLectorEstadoPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarEfectividadTomaLectorEstadoExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-lector-toma-estado.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
