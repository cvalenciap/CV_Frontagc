...
import { ParametrosCargaBandeja, RepoEfecInspeComer, Paginacion, Response, Periodo, Ciclo, TipoInspe } from 'src/app/models';
...
@Component({
  selector: 'app-efectividad-inspeccion-comercial',
  templateUrl: './efectividad-inspeccion-comercial.component.html',
  styleUrls: ['./efectividad-inspeccion-comercial.component.scss']
})
export class EfectividadInspeccionComercialComponent implements OnInit {
  ...
  items: RepoEfecInspeComer[];
  itemsPagination: RepoEfecInspeComer[];
  ...
  private consultarParametros() {
    ...
    this.tipoInspeList = JSON.parse(sessionStorage.getItem('listaTipoInspe'));
    this.tipoInspeList.push({TIPO:'', DESC_TIPO: "TODOS"});
  }
  ...
  OnFiltrar() {
    ...
    request.v_tipo_inspe = this.tipoInspe == ''? null : this.tipoInspe;
    this.reportesMonitoreoService.obtenerRepoEfecInspeComer(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecInspeComerPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecInspeComerExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-inspeciones-comerciales.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
