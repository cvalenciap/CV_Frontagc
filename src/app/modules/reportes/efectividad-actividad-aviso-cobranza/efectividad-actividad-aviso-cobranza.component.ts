...
import { ParametrosCargaBandeja, RepoEfecActiAvisCob, Paginacion, Response, Periodo, Ciclo } from 'src/app/models';
...
@Component({
  selector: 'app-efectividad-actividad-aviso-cobranza',
  templateUrl: './efectividad-actividad-aviso-cobranza.component.html',
  styleUrls: ['./efectividad-actividad-aviso-cobranza.component.scss']
})
export class EfectividadActividadAvisoCobranzaComponent implements OnInit {
  ...
  items: RepoEfecActiAvisCob[];
  itemsPagination: RepoEfecActiAvisCob[];
  ...
  OnFiltrar() {
    ...
    this.reportesMonitoreoService.obtenerRepoEfecActiAvisCob(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecActiAvisCobPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecActiAvisCobExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-efectividad-aviso-cobranza.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
