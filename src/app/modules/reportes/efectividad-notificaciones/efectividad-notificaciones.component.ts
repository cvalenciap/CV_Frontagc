...
import { ParametrosCargaBandeja, RepoEfecNotificaciones, Paginacion, Response, Periodo, Ciclo } from 'src/app/models';
...
@Component({
  selector: 'app-efectividad-notificaciones',
  templateUrl: './efectividad-notificaciones.component.html',
  styleUrls: ['./efectividad-notificaciones.component.scss']
})
export class EfectividadNotificacionesComponent implements OnInit {
  ...
  items: RepoEfecNotificaciones[];
  itemsPagination: RepoEfecNotificaciones[];
  ...
  OnFiltrar() {
    ...
    this.reportesMonitoreoService.obtenerRepoEfecNotificaciones(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarEfectividadNotificacionesPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarEfectividadNotificacionesExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-efectividad-notificaciones.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
