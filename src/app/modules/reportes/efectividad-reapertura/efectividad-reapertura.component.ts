...
import { ParametrosCargaBandeja, RepoEfecApertura, Paginacion, Response, Periodo, Ciclo } from 'src/app/models';
...
@Component({
  selector: 'app-efectividad-reapertura',
  templateUrl: './efectividad-reapertura.component.html',
  styleUrls: ['./efectividad-reapertura.component.scss']
})
export class EfectividadReaperturaComponent implements OnInit {
  ...
  items: RepoEfecApertura[];
  itemsPagination: RepoEfecApertura[];
  ...
  OnFiltrar() {
    ...
    this.reportesMonitoreoService.obtenerRepoEfecApertura(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecAperturaPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecAperturaExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-efectividad-apertura.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
