...
import { ParametrosCargaBandeja, RepoEfecInspeInt, Paginacion, Response, Periodo, Ciclo, TipoInspe } from 'src/app/models';
...
@Component({
  selector: 'app-efectividad-inspeccion-interna',
  templateUrl: './efectividad-inspeccion-interna.component.html',
  styleUrls: ['./efectividad-inspeccion-interna.component.scss']
})
export class EfectividadInspeccionInternaComponent implements OnInit {
  ...
  items: RepoEfecInspeInt[];
  itemsPagination: RepoEfecInspeInt[];
  ...
  OnFiltrar() {
    ...
    this.reportesMonitoreoService.obtenerRepoEfecInspeInt(request).subscribe(
      (response: Response) => {
  ...
  OnPDF() {
    ...
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecInspeIntPDF(this.items).subscribe(
      (data) => {
  ...
  OnExcel() {
    const this_ = this;
    this.loading = true;
    this.reportesMonitoreoService.generarRepoEfecInspeIntExcel(this.items).subscribe(
      (data) => {
        const file = new Blob([data], { type: TipoArchivo.xlsx });
        this_.fileService.downloadFile(file, 'informe-inspeciones-internas.xlsx');
        this.loading = false;
      },
      (response: Response) => this.controlarError(response),
    );
