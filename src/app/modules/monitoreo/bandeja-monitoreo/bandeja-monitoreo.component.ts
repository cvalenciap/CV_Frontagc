...
export class BandejaMonitoreoComponent implements OnInit {
  ...
  public tomaEstado: TomaEstado[];
  public tomaEstadoCopy: TomaEstado[];
  ...
  public mostrarTE: boolean;
  public mostrarIC: boolean;
  public mostrarDC: boolean;
  public mostrarDA: boolean;
  public mostrarVarios: boolean;
  ...
  ngOnInit() {
    ...
    this.monitoreoService.consultarParametrosBusquedaMonitoreo('TE').subscribe(
    ...
    this.monitoreoService.obtenerParametrosPeriodo().subscribe(
      ...
      this.mostrarDA = false
      this.mostrarIC = false
      this.mostrarDC = false
      this.mostrarTE = false
      this.mostrarVarios = false
  ...
  seleccionarViewCiclo(event) {
    if (event) {
      this.filtros.ciclo.detalle = event.detalle;
    } else {
      this.filtros.ciclo = new Parametro();
    }
  }
  onSeleccionarPeriodo(event) {
    if (event) {
      this.filtros.periodo.detalle = event.detalle;

      this.monitoreoService.consultarParametroCiclo(event.detalle, this.filtros.oficina, "TE").subscribe(
        (response: ResponseObject) => {
          this.listaParametros.listaCiclo = response.resultado.listaCiclo;
        }
      );

      this.comboCiclo = true;
    } else {
      this.filtros.periodo = new Parametro();
      this.filtros.ciclo = new Parametro();
      this.comboCiclo = false;
  ...
  validarFormularioDetalle(filtros: FiltrosBandejaMonitoreoDetalle) {
    ...
    private mostrarInformacionFiltros() {
      ...
      if (this.filtrosBusqueda.imposibilidad) {
        mensaje = mensaje + '<br/><strong>Incidencia: </strong> <parrafo>' + this.filtrosBusqueda.imposibilidad.detalle + '</parrafo>' + ' ';
        this.mostrarFiltros = true;
      }
      ...
      if (this.filtrosBusqueda.semaforo) {
        mensaje = mensaje + '<br/><strong>Semáforo: </strong> <parrafo>' + this.filtrosBusqueda.semaforo.detalle + '</parrafo>' + ' ';
        this.mostrarFiltros = true;
      }
    ...
    eliminarFiltrosDetalle(){
      ...
      this.tomaEstado = this.tomaEstadoCopy;
    ...
    this.monitoreoService.obtenerDetallesMonitoreo(monitoreo).subscribe(
      (response: ResponseObject) => {
        if (!(response.resultado.listarMonitoreoDetalle.length > 0)) {
          this.loading = false;
          this.tomaEstado = new Array<TomaEstado>();
        ...
        this.tomaEstado = response.resultado.listarMonitoreoDetalle;
        this.tomaEstadoCopy = response.resultado.listarMonitoreoDetalle;
  ...
  onObtenerDetalles() {
    ...
    if (this.filtrosDetalle.periodo.codigo) {
      this.filtrosBusquedaDetalle.periodo = this.filtrosDetalle.periodo;
    } else {
      this.filtrosBusquedaDetalle.periodo = null;
    }
    ...
    this.monitoreoService.obtenerDetallesBusqueda(this.filtrosBusquedaDetalle).subscribe(
      (response: ResponseObject) => {
        if (!(response.resultado.listarMonitoreoDetalle.length > 0)) {
          this.loading = false;
          this.tomaEstado = new Array<TomaEstado>();
...
/*
    this.filtrosBusqueda.periodo.codigo = this.filtros.periodo.codigo;
    this.filtrosBusqueda.ciclo.codigo = this.filtros.ciclo.codigo; */
    ...
    if (this.filtros.periodo.detalle) {
      this.filtrosBusqueda.periodo = this.filtros.periodo;
    } else {
      this.filtrosBusqueda.periodo = null;
    }
    if (this.filtros.ciclo.detalle) {
      this.filtrosBusqueda.ciclo = this.filtros.ciclo;
    } else {
      this.filtrosBusqueda.ciclo = null;
    }
    ...
    if (sessionStorage.getItem('codOficina') != null) {
      ...
      this.filtrosBusqueda.oficina.descripcion = sessionStorage.getItem('descOficina');
    } else {
    ...
    if (this.filtrosBusqueda) {
      this.monitoreoService.obtenerMonitoreos(this.filtrosBusqueda, 'TE').subscribe(
        (response: ResponseObject) => {
  ...
  visualizarImagen(item: TomaEstado, accion: string): void{
      this.loading = true;
  ...
  onExportarDetalle(){
    this.monitoreoService.generaExcelDetalle(this.cabecera, this.tomaEstado, "TE").subscribe(
      (data) => {
        this.loading = false;
  ...
  onExportar(){
    this.monitoreoService.generaExcel(this.filtrosBusqueda, "TE").subscribe(
      (data) => {
        this.loading = false;
        var file = new Blob([data], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
        saveAs(file, "Resultado");
