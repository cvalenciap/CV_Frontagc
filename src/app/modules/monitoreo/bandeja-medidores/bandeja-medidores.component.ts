...
@Component({
  selector: 'app-bandeja-medidores',
  templateUrl: './bandeja-medidores.component.html',
  styleUrls: ['./bandeja-medidores.component.scss']
})

export class BandejaMedidoresComponent implements OnInit {
  name = 'Angular';
  ...
  constructor(private sanitizer: DomSanitizer,
    ...
    this.filtrosDetalle.tipoCarga = new Parametro();
    this.filtrosDetalle.tipoInstalacion= new Parametro();
  ...
  ngOnInit() {
    ...
    this.monitoreoService.consultarParametrosBusquedaMonitoreo('ME').subscribe(
  ...
  onListarDetalles(): Medidor[] {
    ...
    return this.medidor.slice(inicio, fin);
  }
  ...
  onSeleccionarZonaDetalle(event) {
    if (event) {
  ...
  onSeleccionarTipoCarga(event) {
    if (event) {
      this.filtrosDetalle.tipoCarga.detalle = event.detalle;
    } else {
      this.filtrosDetalle.tipoCarga = new Parametro();
    }
  }
  onSeleccionarTipoInstalacion(event) {
    if (event) {
      this.filtrosDetalle.tipoInstalacion.detalle = event.detalle;
    } else {
      this.filtrosDetalle.tipoInstalacion = new Parametro();
    }
  }
  ...
  onSeleccionarCodObrsevacion(event) {
    if (event) {
  ...
  validarFormularioDetalle(filtros: FiltrosBandejaMonitoreoDetalle) {
    ...
    private mostrarInformacionFiltrosDetalle() {
      ...
      if (this.filtrosBusquedaDetalle.medidorInstalado) {
        mensaje = mensaje + '<br/><strong>Medidor Instalado: </strong> <parrafo>' + this.filtrosBusquedaDetalle.medidorInstalado + '</parrafo>' + ' ';
        this.mostrarFiltrosDetalle = true;
      }
      if (this.filtrosBusquedaDetalle.medidorRetirado) {
        mensaje = mensaje + '<br/><strong>Medidor Retirado: </strong> <parrafo>' + this.filtrosBusquedaDetalle.medidorRetirado + '</parrafo>' + ' ';
        this.mostrarFiltrosDetalle = true;
      }
      ...
      if (this.filtrosBusquedaDetalle.codObservacion) {
        mensaje = mensaje + '<br/><strong>Código de Observación: </strong> <parrafo>' + this.filtrosBusquedaDetalle.codObservacion.detalle + '</parrafo>' + ' ';
        this.mostrarFiltrosDetalle = true;
      }
      if (this.filtrosBusquedaDetalle.tipoCarga) {
        mensaje = mensaje + '<br/><strong>Tipo de Carga: </strong> <parrafo>' + this.filtrosBusquedaDetalle.tipoCarga.detalle + '</parrafo>' + ' ';
        this.mostrarFiltrosDetalle = true;
      }
      if (this.filtrosBusquedaDetalle.tipoInstalacion) {
        mensaje = mensaje + '<br/><strong>Tipo de Instalación: </strong> <parrafo>' + this.filtrosBusquedaDetalle.tipoInstalacion.detalle + '</parrafo>' + ' ';
        this.mostrarFiltrosDetalle = true;
      }
    ...
    eliminarFiltrosDetalle(){
      ...
      this.medidor = this.medidorCopy;
      this.paginacion2 = new Paginacion({
        pagina: 1,
        registros: 10,
        totalRegistros: this.medidor.length,
        totalPaginas: this.medidor.length
      })
    }
    ...
    this.monitoreoService.obtenerDetallesMonitoreo(monitoreo).subscribe(
      (response: ResponseObject) => {
        if (!(response.resultado.listarMonitoreoDetalle.length > 0)) {
          this.loading = false;
          this.medidor = new Array<Medidor>();
        ...
        this.medidor = response.resultado.listarMonitoreoDetalle;
        this.medidorCopy = response.resultado.listarMonitoreoDetalle;
  ...
  onObtenerDetalles() {
    ...
    this.filtrosBusquedaDetalle.tipoCarga = new Parametro();
    this.filtrosBusquedaDetalle.tipoInstalacion = new Parametro();
    ...
    if (this.filtrosDetalle.tipoCarga.codigo) {
      this.filtrosBusquedaDetalle.tipoCarga = this.filtrosDetalle.tipoCarga;
    } else {
      this.filtrosBusquedaDetalle.tipoCarga = null;
    }
    if (this.filtrosDetalle.tipoInstalacion.codigo) {
      this.filtrosBusquedaDetalle.tipoInstalacion = this.filtrosDetalle.tipoInstalacion;
    } else {
      this.filtrosBusquedaDetalle.tipoInstalacion = null;
    }
    ...
    this.filtrosBusquedaDetalle.medidorInstalado = this.filtrosDetalle.medidorInstalado;
    this.filtrosBusquedaDetalle.medidorRetirado = this.filtrosDetalle.medidorRetirado;
    ...
    this.monitoreoService.obtenerDetallesBusquedaME(this.filtrosBusquedaDetalle).subscribe(
      (response: ResponseObject) => {
        if (!(response.resultado.listarMonitoreoDetalle.length > 0)) {
          this.loading = false;
          this.medidor = new Array<Medidor>();
  ...
  onObtenerMonitoreo() {
    ...
    if (this.filtrosBusqueda) {
      this.monitoreoService.obtenerMonitoreos(this.filtrosBusqueda, 'ME').subscribe(
        (response: ResponseObject) => {
  ...
  onLimpiarFiltrosDetalle() {
    ...
    this.filtrosDetalle.tipoActividad = new Parametro();
    this.filtrosDetalle.zona = new Zona();
    this.filtrosDetalle.medidorInstalado = null;
    this.filtrosDetalle.foto = new Parametro();
    this.filtrosDetalle.suministro = null;
    this.filtrosDetalle.cumplimiento = new Parametro();
    this.filtrosDetalle.ordenServicio = null;
    this.filtrosDetalle.medidorRetirado = null;
    this.filtrosDetalle.tipoOrdenServicio = new Parametro();
    this.filtrosDetalle.codObservacion = new Parametro();
    this.filtrosDetalle.tipoCarga = new Parametro();
    this.filtrosDetalle.tipoInstalacion = new Parametro();
  ...
  visualizarImagen(item: Medidor, accion: string): void{
      this.loading = true;
  ...
  onExportarDetalle(){
    this.monitoreoService.generaExcelDetalle(this.cabecera, this.medidor, "ME").subscribe(
      (data) => {
        this.loading = false;
  ...
  onExportar(){
    this.monitoreoService.generaExcel(this.filtrosBusqueda, "ME").subscribe(
      (data) => {
        this.loading = false;
        var file = new Blob([data], {type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'});
        saveAs(file, "Resultado");
