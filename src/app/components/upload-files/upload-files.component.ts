...
export class UploadFilesComponent implements OnInit {
  ...
  listaForm: FormData[];
  loading = false;
  constructor(private http: HttpClient, private service: CargasTrabajoService) {
    this.apiEndpoint = environment.serviceEndpoint + '/piloto';
    this.listaForm = [];
    this.path = {
      saveUrl: `${this.apiEndpoint}/documentos`,
  ...
  subir() {
    this.loading = true;
          if (this.dropElement.querySelector('ul') && this.filesDetails.length > 0) {
              ...
              const cantArchivos = this.filesDetails.length;
            ...
            for (let i = 0; i < vueltas; i ++) {
                ...
                for (let j = (i * 500); j < fin; j++) {
                    formData.append('files', this.filesDetails[j].rawFile, this.filesDetails[j].rawFile['name']);
                }
            ...
            for (let i = 0; i < vueltas; i ++) {
                 this.loading = true;
                this.subirAsync(this.listaForm[i]).then( x => {
                    this.loading = false;
                }).catch(error => {
                    this.loading = false;
                });
  ...
  async subirAsync(data: FormData) {
    const rpta = await this.http.post(`${this.apiEndpoint}/documentos`, data).toPromise();
  }
  ...
  procesar() {
    ...
    this.service.procesarTrama().subscribe(
        (response: Response) => {
